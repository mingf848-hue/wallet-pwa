...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!imageBase64 || !transactionTime) {
      // 检查：如果连时间都没传过来，那肯定是快捷指令没配好
      return res.status(400).json({ error: '缺少图片或交易时间(transactionTime)，请检查快捷指令配置。' });
    }

    const currentYear = new Date().getFullYear();
    // 将 iPhone 传来的时间作为基准时间，精确到毫秒
    const iphoneBaseTime = new Date(transactionTime); 

    // --- A. Prompt: 保留人设，要求 AI 提取日期 ---
    const systemPrompt = `
      ...
      类型 B：账单列表页 -> 返回多条记录。
      category (分类)：从 food, shop, transport, home, fun, other 中选一个最准确的。
      platform (平台)：根据界面颜色判断 (绿色=WeChat, 蓝色=Alipay, 红色/银联=UnionPay)。

      返回 JSON 示例：
      [
        {"amount": 135.03, "type": "expense", "merchant": "蜜雪冰城", "product_name": "生菜, 瓜子...", "category": "food", "platform": "WeChat", "date": "2025-12-10 18:00:00"},
    ...
    for (const item of items) {
        ...
        // ★★★ 核心逻辑：时间优先级判断 ★★★
        let finalTxDate = new Date(iphoneBaseTime); // 默认使用 iPhone 传来的时间
